...
# %%
yagotypes.lazy().filter(pl.col("cat_object").is_in(top10types["cat_object"])).select(
...
# %%
yagofacts_frequenttypes = (
    yagofacts.lazy()
    .filter(pl.col("subject").is_in(most_frequent_types["subject"]))
    .collect()
)
...
# %%
cooccurring_predicates = (
    yagofacts_frequenttypes.lazy()
    .join(
        yagofacts_frequenttypes.lazy(),
        left_on="subject",
        right_on="subject",
    ...
    .groupby("predicate_right")
    .agg(pl.first("predicate"))
    .collect()
)
...
# %%
count_cooccurring_predicates = (
    cooccurring_predicates.lazy()
    .groupby(["predicate", "predicate_right"])
    .agg(pl.count())
    .sort("count", descending=True)
    .collect()
...
types_predicates = (
    yagotypes.lazy()
    .filter(pl.col("cat_object").is_in(top10types["cat_object"]))
...
groups = (
    yagotypes.lazy()
    .join(top10types.lazy().select(pl.col("cat_object")), on="cat_object", how="inner")
    .groupby(pl.col("cat_object"))
    .all()
    .select([pl.col("cat_object"), pl.col("subject")])
    .collect()
)
...
for type_str, tab in tqdm(tabs_by_type.items(), total=len(tabs_by_type)):
    ...
    for pred_name, pred_group in yagofacts.groupby("predicate"):
        if pred_name in G.neighbors(type_str) and pred_name in count_facts["predicate"]:
            ...
            full_tables_by_type[type_str] = (
                full_tables_by_type[type_str]
                .lazy()
                .join(transformed_tab.lazy(), on="subject", how="left")
            )
